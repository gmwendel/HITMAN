...
@tf.function
def tfLLH_joint_factorized(charges, pmt_positions, hyp_batch, yields, shape_net, acc_net, b_val=0.1):
    """
    ...
    # hyp_batch is (N_hyp, 3) where params are E, Scat, Abs
    # Note: For our new factorized network, the energy is technically decoupled,
    # but we still pass it inside the hypothesis array and use `yields` for absolute scale.
    
    # We need to broadcast the hypothesis and PMT positions for the networks
    # Since ShapeNet takes (N_hyp, N_sensors, 3) for PMTs
    ...
    # Lambda_sig_exp: (N_hyp, N_events, 1)
    ...
    # --- Step B: Vertex Optimization (TODO) ---
    # TODO: Inject parallel vertex maximization here. 
    # For now, we bypass it since we assume fixed vertices in the grid search.
    
    # Evaluate LLH
    # lam_{i,j} = eps * S_{i,j} + b_i
    eps_broadcast = tf.expand_dims(eps, axis=-1) # (N_hyp, 1, 1)
    b_broadcast = tf.reshape(b, (1, 1, N_sensors))
    
    lam = eps_broadcast * S + b_broadcast
    
    # LLH = sum( k * ln(lam) - lam )
    ...
    # We return the negative log-likelihood (NLL) for minimization
    return -llhs
