...
def ntot_block(ntot) -> dict:
    """Serialize a :class:`hitman.receipts.forward.NTotResult`."""
...
def ring_time_block(ring) -> dict:
    """Serialize a :class:`hitman.receipts.forward.RingTimeResult`."""
    return {
...
def mle_block(m) -> dict:
    """Serialize a :class:`hitman.receipts.mle.MLEResult`."""
    return {
        "per_param": {
            p.name: {
                "bias": _f(p.bias),
