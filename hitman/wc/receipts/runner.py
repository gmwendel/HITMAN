...
:mod:`hitman.wc.receipts.harness` (DESIGN proposal 3) and the numeric receipt logic in the
sibling modules (``reweight``, ``chi2``, ``forward``, ``score``, ``mle``). ``run_model_dir``
is the WC instantiation: it wires an :class:`~hitman.wc.receipts.harness.NREReceiptModel` and
the default ``z_rings`` strata through
:func:`~hitman.wc.receipts.harness.forward_block_from_model`. The generic seam
(:func:`~hitman.wc.receipts.harness.compute_forward_block`) is re-exported here so the
historical ``from hitman.wc.receipts.runner import compute_forward_block`` path is unchanged.
"""
...
from hitman.wc.receipts.harness import (  # noqa: F401
    NREReceiptModel,
    ReceiptModel,
