...
:data:`WC_OBS_SPEC`, kept bit-for-bit consistent with :mod:`hitman.nn.features` and
...
class HypSpec(NamedTuple):
    ...
    def direction(self, theta: jnp.ndarray) -> jnp.ndarray:
        ...
        Bit-for-bit ``hitman.nn.features.direction`` when ``zenith``/``azimuth`` point at
...
# to hitman.nn.features (X, Y, Z, ZENITH, AZIMUTH, TIME, ENERGY); test_spec locks them.
WC_HYP_SPEC = HypSpec(
    dims=(
