def main():
    ...
    parser.add_argument('-i', '--input_files', help='Type = String; locations of events to be evaluated, e.g. $PWD/{1..16}.pkl', nargs='+', required=True)
    parser.add_argument('-n', '--network', help='Type = String array; Location of trained network to be used for reconstruction, e.g. $PWD/hitnet.h5, $PWD/chargenet.h5', nargs=2, required=True)
    parser.add_argument('-o', '--output_file', help='Type = String;  Specify output file with reconstructed values; e.g. $PWD/reco.pkl',nargs=1, required=True)
    parser.add_argument('-r', '--radius', help='Type = int;  Specify detector radius in mm',nargs=1, required=True)
    parser.add_argument('-z', '--half_height', help='Type = int;  Specify detector half-height in mm',nargs=1, required=True)
    parser.add_argument('--print_numpy', default=False, type=bool, help='Type = Boolean;  Prints additional numpy files about failed events, etc.; Default = False')

    args = parser.parse_args()

    import numpy as np

    from hitman.neural_nets.transformations import chargenet_trafo, hitnet_trafo
    ...
    #    descent_rates=tf.tile([[600.,600.,600.,0.1,0.1,0.00013,0.1]],(len(params),1))*160/(len(hits)+15)*0.1 #gentle t
        descent_rates=tf.tile([[800.,800.,800.,0.01,0.01,0.013,0.003]],(len(params),1))*0.25*95/(len(hits)+15) #water best
    ...
    def calc_n9(event,theta):
        ...
        x=theta
        hit=event['hits']
        residuals=(hit[:,3]-x[5]) - n/c*((x[0]-hit[:,0])**2 + (x[1]-hit[:,1])**2 + (x[2]-hit[:,2])**2)**0.5
        lower=-3
        upper=6
    ...
    for event in events:
        #generate 'best guess'
        initial_points=best_guess(hitnet,chargenet,event,final_number,samples,float(args.half_height[0]),float(args.radius[0]),-5,5,2.0)
